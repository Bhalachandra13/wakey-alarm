...
void main() {
  ...
  group('EditAlarmScreen with location trigger', () {
    ...
    testWidgets('switching to location reveals map picker section', (
      ...
      // The search field is NOT in the edit screen anymore — it
      // moved into the map picker. The edit screen now has just
    ...
    testWidgets('editing an existing location alarm pre-fills the form', (
      ...
      // The button to launch the picker now reads "Change on map"
      // when a location is already picked.
      expect(find.text('Change on map'), findsOneWidget);
    });

    testWidgets(
      'tapping "Pick on map" launches MapPickerScreen with the search field',
      (tester) async {
        await tester.pumpWidget(wrap());
        await tester.pumpAndSettle();

        // Switch to location mode and tap the "Pick on map" button.
        await tester.tap(find.text('Location'));
        await tester.pumpAndSettle();
        await tester.tap(find.text('Pick on map'));
        await tester.pumpAndSettle();

        // The map picker is on screen.
        expect(find.text('Pick location'), findsOneWidget);
        // And its search field is visible from the start.
    ...
    testWidgets(
      'clearing the location resets lat/lon but leaves the screen usable',
      (tester) async {
        const existing = Alarm(
          id: 1,
          label: 'Train stop',
        ...
        // The lat/lon display falls back to the empty state.
        expect(find.text('No location picked yet'), findsOneWidget);
        // The radius slider is now disabled again.
        final slider = tester.widget<Slider>(find.byType(Slider));
        expect(slider.onChanged, isNull);
        // The "Clear" button has been replaced with a no-op since
        // there's nothing to clear.
        expect(find.widgetWithText(OutlinedButton, 'Clear'), findsNothing);
        // The picker-launch button now reads "Pick on map" again
        // (the original wording for a fresh start).
        expect(find.text('Pick on map'), findsOneWidget);
      },
    );
  });
