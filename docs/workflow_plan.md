...
## Iteration 5 — Phase D: Play Store publishing prep

*This section was added once the Iteration 5 features were code-
complete. It is a separate, smaller iteration because the
publishing prep doesn't touch the app's runtime behaviour — it
touches the Android build chain, the Play Console-facing
metadata, and the host of static documents Play requires
(privacy policy, Data Safety answers, permission
justifications). The runtime feature work is the prior
section.*

### Tasks

- [x] **Upload keystore.** Generated `wakey-upload-key.jks` —
  RSA 4096, 9125-day validity, alias `upload`, DN
  `CN=TingerBuddanna, OU=Wakey-Wakey, O=TingerBuddanna`. PKCS12
  format (the JDK 9+ default — `keytool` does not support
  separate key passwords under PKCS12, so `keyPassword ==
  storePassword` in `key.properties`).
- [x] **`android/key.properties`** with `storeFile`, `keyAlias`,
  `storePassword`, `keyPassword`. Both `key.properties` and
  `*.jks` are gitignored at the repo root and again under
  `android/.gitignore` (defence in depth).
- [x] **Signing config in `android/app/build.gradle.kts`.** The
  release `buildType` now signs with the upload keystore
  (previously signed with the debug key, which Play Console
  rejects). Reads credentials from `key.properties`; falls
  back to the debug signing config when the file is absent so
  contributors without a checkout of the keystore can still
  build the dev APK.
- [x] **Backup rules** — `android/app/src/main/res/xml/backup_rules.xml`
  (API 23-30) and `data_extraction_rules.xml` (API 31+).
  Includes the user's alarms, timers, and favourites via
  Auto Backup / device transfer; no `cache` exclude (the
  domain is invalid in the schema and the platform never
  backs up caches by default).
- [x] **Manifest updates** — `android:roundIcon` on the
  application tag (Play Console insists on it), `dataExtractionRules`
  and `fullBackupContent` pointing at the two XML files.
- [x] **Adaptive launcher icon** — `mipmap-anydpi-v26/ic_launcher.xml`
  + `ic_launcher_round.xml` referencing a background colour
  (`values/colors.xml`, `ic_launcher_background = #1E3A8A`)
  and a foreground drawable. The foreground is a 432×432
  PNG rendered by `tools/generate_launcher_icon.py`. The
  legacy mipmap-{m,h,xh,xxh,xxxh}dpi PNGs are also
  generated, for launchers on API 26-27 that prefer the
  static fallback. (Note: the existing mipmap PNGs in the
  repo from the Flutter `create` scaffold have been
  overwritten by the regenerated ones — the new design is
  the canonical launcher art.)
- [x] **`pubspec.yaml` description** updated from the Flutter
  scaffold's "A new Flutter project." to a real product
  description. The description appears in `flutter pub` and
  is the only public description in the pub.dev metadata
  (the package is not published to pub.dev, but the field
  is also shown by IDE tooling).
- [x] **Privacy policy** — `docs/privacy-policy.md`. Drafted
  in-repo with accurate descriptions of every on-device
  data category, the third-party SDKs (Google Play Services
  Location, Maps), and a contact email placeholder. Ready to
  host on GitHub Pages (free); a one-time edit replaces the
  placeholder email before going live.
- [x] **Data Safety answers** — `docs/data-safety.md`. Maps
  every Play Console "Data Safety" form question to the
  correct answer with a code-reference justification. The
  only collected data type is precise location, marked as
  collected-only (not shared) and used solely for the
  geofence feature.
- [x] **Permission justifications** — `docs/permission-justifications.md`.
  Paste-ready text for every runtime permission the
  manifest declares, written to match the strict framing
  the Play policy team now requires (background location
  and exact alarms in particular). Includes the
  cheat-sheet for quick copy-paste and the long-form
  version for if/when the review team pushes back.
- [x] **Publish runbook** — `docs/play-store-publish.md`.
  Step-by-step walkthrough of the Play Console from
  pre-flight through post-publish, including how to host
  the privacy policy, capture screenshots, fill the Data
  Safety form, declare each permission, run the staged
  rollout, and what to do on rejection.
- [x] **Store assets** — `store_assets/icon-512.png` and
  `store_assets/feature-graphic-1024x500.png`, both
  generated by `tools/generate_launcher_icon.py`. The
  script is a pure-stdlib Python program (no Pillow, no
  ImageMagick) so it can be re-run from any contributor's
  machine. `store_assets/README.md` documents the
  directory and explains where to put screenshots.
- [x] **Signed AAB** — `build/app/outputs/bundle/release/app-release.aab`
  (54.1 MB). Built with `flutter build appbundle --release`.
  Verified signed: `META-INF/UPLOAD.RSA` extracted and
  inspected with `keytool -printcert` — owner matches
  the upload keystore, SHA-1 `2F:C3:08:53:...`, 4096-bit
  RSA, valid until 2051. R8 + resource shrinking are
  active in the build script (the prior `--no-shrink`
  workaround is no longer needed because the ProGuard
  keep rules for every native bridge class are in place
  and were verified in the Iter 5 groundwork commit).
- [x] **`splits.abi` removed from `build.gradle.kts`.** The
  AAB and per-ABI splits are mutually exclusive (see
  https://issuetracker.google.com/402800800). The splits
  were a dev-time convenience for direct sideloading; for
  Play distribution, Google splits per-ABI internally. If
  a lean per-ABI APK is ever needed for sideloading,
  `flutter build apk --split-per-abi` enables the splits
  on the APK build path without changing the AAB build.

### Dependencies

None at runtime — this work does not touch Dart or Kotlin
code. Build-script changes (signing config, `splits` removal)
are validated by the AAB build. Documentation changes are
validated by reading them and (for the runbook) by the
human following the steps.

### Definition of Done

- [x] **Automated:** `flutter analyze` clean.
- [x] **Automated:** `flutter test` — 328 tests pass.
- [x] **Automated:** `flutter build appbundle --release` —
      signed AAB is produced; signature verified via
      `keytool -printcert` on `META-INF/UPLOAD.RSA`.
- [ ] **Manual (on-device):** The underlying Iteration 5
      features (favourite-locations flow, permission
      setup wizard, geofence boot re-arm) — see the
      existing Iteration 5 DoD checkboxes above. **These
      are still pending human verification on a physical
      device, and the publishing prep does not close
      them out** — the publishing prep is the production-
      sign-off scaffolding around them.
- [ ] **Manual (Play Console):** Follow
      `docs/play-store-publish.md` to fill the Console
      form, upload the AAB, and start the rollout. This
      step requires the $25 Play Developer registration,
      a hosted privacy policy URL, and real device
      screenshots — none of which the assistant can
      perform. The runbook is structured so the human can
      work through the Console without further
      assistance.

### Scope note

Per AGENTS.md §8.3 the manual on-device checklist is
**never** optional. The publishing prep adds an analogous
"manual Play Console walkthrough" step that only the human
can perform — it requires a real Play Developer account,
payment information, and access to a physical device for
screenshot capture. The runbook is written to be
self-sufficient: paste the justifications, upload the AAB,
click through the form, done.

The Maps API key situation worth flagging: a real key is
already present in `android/local.properties`
(gitignored), so the AAB is built with a real key and the
listing icon's map will render. The runbook still walks
through the Google Cloud Console restriction setup
(package name + upload-keystore SHA-1) so the key can't
be abused if it leaks.
